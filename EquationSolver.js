...
export class EquationSolver {
  ...
  static ERROR_INVALID_DECIMAL() {
    return EquationSolver.ERROR("Invalid decimal");
  }
  ...
  infixToPostfix(infix) {
    ...
    if (!/^[\d+\-*/^()\s]*$/.test(infix) || infix.trim() == "") {
      ...
      for (let i = 0; i < infix.length; i++) {
        ...
        if (!isNaN(char)) {
          postfix += char;
          // if the character is an o6perator
...
/**
 * This function converts an infix notation to postfix notation using the Shunting-yard algorithm
 * It can handle, decimal numbers, multiple digits, parenthesis, and the following operators: +, -, *, /, ^
 * @param {string} infix - the infix notation to convert
 * @return {string} postfix - the postfix notation
 */
// infixToPostfix(infix) {
...
//     for (let i = 0; i < infix.length; i++) {
//       let char = infix[i];
//       // if the character is a number or a decimal point, add it to the postfix notation
//       if (!isNaN(char) || char === ".") {
//         postfix += char;
//         // if the next character is a decimal point, throw an error
//         if (char === "." && !isNaN(infix[i + 1])) {
//           this.setResult(EquationSolver.ERROR_INVALID_DECIMAL());
//         }
//         // if the next character is an operator in the precedence object, add a space after it
//         if (infix[i + 1] in precedence) {
//           postfix += " ";
//         }
